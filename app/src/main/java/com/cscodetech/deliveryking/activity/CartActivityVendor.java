...
public class CartActivityVendor extends AppCompatActivity implements GetResult.MyListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myHelper = new DatabaseHelper(CartActivityVendor.this);
        ...
        itemList = myHelper.getAllData();

        adapter = new CartAdpVendor(this, itemList);
        ...
        chWallet.setOnCheckedChangeListener((buttonView, isChecked) -> CartActivityVendor.this.updateCartData());
    ...
    private void getHome(String rid) {
        ...
        Call<JsonObject> call = APIClient.getInterface().getCartStore(bodyRequest);
    ...
    public void updateCount() {
        adapter = new CartAdpVendor(this, myHelper.getAllData());
        recyclerCart.setAdapter(adapter);
        updateCartData();
    }
    ...
    public void selectLocation(Context context, List<MyAddress> list) {
        ...
        txtAddaddress.setOnClickListener(v -> {
            ...
            startActivity(new Intent(CartActivityVendor.this, MapActivity.class));
        });
    ...
    public void updateCartData() {
        ...
        itemtotal = 0;
        double aprice = 0;
        List<MyCart> itemList = myHelper.getAllData();
        if (itemList.size() != 0) {
            lvlBottom.setVisibility(View.VISIBLE);
            lvlMain.setVisibility(View.VISIBLE);
            lvlNotfound.setVisibility(View.GONE);
            ...
            for (int i = 0; i < itemList.size(); i++) {
                MyCart item = myHelper.getAllData().get(i);
                aprice = 0;

                double temptotal = aprice;
                double productPrice= Double.valueOf(item.getProductPrice());
                double discountPercent= Double.valueOf(item.getDiscount());
                if(discountPercent > 0.0) {
                    productPrice = productPrice - ((discountPercent / 100) * productPrice) ;
                }
                itemtotal = itemtotal + (temptotal + (Integer.parseInt(item.getQty()) * productPrice));
            }
        ...
        switch (view.getId()) {
            ...
            case R.id.txt_process:
                if (Integer.parseInt(cart.getRestuarantData().get(0).getRestMorder()) <= itemtotal) {
                    ...
                    Toast.makeText(CartActivityVendor.this, "Minimum order amount " + cart.getRestuarantData().get(0).getRestMorder(), Toast.LENGTH_LONG).show();
                }
                break;
            case R.id.lvl_storeclose:
                ...
                break;
            case R.id.btn_login:
                startActivity(new Intent(CartActivityVendor.this, LoginActivity.class));
                break;
            case R.id.img_coopncode:
                if (sessionManager.getIntData(coupon) != 0) {
                    ...
                    startActivity(new Intent(CartActivityVendor.this, CoupunActivity.class).putExtra("rid", rid).putExtra("amount", temp).putExtra("type","store"));
                }
                break;
            default:
    ...
    public void bottonPaymentList() {
        ...
        for (int i = 0; i < paymentList.size(); i++) {
            LayoutInflater inflater = LayoutInflater.from(CartActivityVendor.this);
            ...
            Glide.with(CartActivityVendor.this).load(APIClient.baseUrl + "/" + paymentList.get(i).getmImg()).thumbnail(Glide.with(CartActivityVendor.this).load(R.drawable.emty)).into(imageView);
            int finalI = i;
                ...
                try {
                    switch (paymentList.get(finalI).getmTitle()) {
                        case "Razorpay":
                            ...
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, RazerpayActivity.class).putExtra("amount", temtoal).putExtra("detail", paymentItem));
                            break;
                        ...
                        case "Paypal":
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, PaypalActivity.class).putExtra("amount", total).putExtra("detail", paymentItem));
                            break;
                        case "Stripe":
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, StripPaymentActivity.class).putExtra("amount", total).putExtra("detail", paymentItem));
                            break;

                        case "FlutterWave":
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, FlutterwaveActivity.class).putExtra("amount", total));
                            break;
                        case "Paytm":
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, PaytmActivity.class).putExtra("amount", total));
                            break;
                        case "SenangPay":
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, SenangpayActivity.class).putExtra("amount", total).putExtra("detail", paymentItem));
                            break;
                        case "PayStack":
                            int temtoal1 = (int) Math.round(total);
                            mBottomSheetDialog.cancel();
                            startActivity(new Intent(CartActivityVendor.this, PaystackActivity.class).putExtra("amount", temtoal1).putExtra("detail", paymentItem));
                            break;
                        default:
                            break;
                    }
        ...
        protected JSONArray doInBackground(String... params) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                try {
                    ...
                    jsonObject.put("is_veg", myCart.getIsVeg());
                    jsonObject.put("attribute_id", myCart.getAttributeId());
                    jsonObject.put("discount", myCart.getDiscount());
                    jsonObject.put("pimg", myCart.getProductImage());
                    jsonObject.put("ptype", myCart.getProductType());
    ...
    public void orderPlace(JSONArray jsonArray) {
        custPrograssbar.prograssCreate(CartActivityVendor.this);
        JSONObject jsonObject = new JSONObject();
        try {
            ...
            jsonObject.put("lats", myAddress.getLatMap());
            jsonObject.put("longs", myAddress.getLongMap());
            jsonObject.put("typeid", cart.getRestuarantData().get(0).getRestType());
            jsonObject.put("ProductData", jsonArray);
            RequestBody bodyRequest = RequestBody.create(MediaType.parse("application/json"), jsonObject.toString());
            Call<JsonObject> call = APIClient.getInterface().getOrderNowStore(bodyRequest);
            GetResult getResult = new GetResult();
            getResult.setMyListener(this);
        ...
        try {
            ...
            if (callNo.equalsIgnoreCase("1")) {
                ...
                if (restResponse.getResult().equalsIgnoreCase("true")) {
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(CartActivityVendor.this);
                    ViewGroup viewGroup = findViewById(android.R.id.content);
                    View dialogView = LayoutInflater.from(CartActivityVendor.this).inflate(R.layout.congratulation_layout, viewGroup, false);
                    TextView txtHome = dialogView.findViewById(R.id.txt_home);
                    TextView txtOrdertrack = dialogView.findViewById(R.id.txt_ordertrack);
                    builder.setView(dialogView);
                    AlertDialog alertDialog = builder.create();
                    alertDialog.setCancelable(false);
                    alertDialog.show();
                    txtHome.setOnClickListener(view -> {
                        alertDialog.cancel();
                        Intent intent = new Intent(CartActivityVendor.this, HomeActivity.class).putExtra("oid", restResponse.getOrderId());
                        startActivity(intent);
                        finish();
                    });
                    txtOrdertrack.setOnClickListener(view -> {
                        alertDialog.cancel();

                        Intent intent = new Intent(CartActivityVendor.this, RestorentOrderActivity.class).putExtra("oid", restResponse.getOrderId());
                        startActivity(intent);
                        finish();
                    });
